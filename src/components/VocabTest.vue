            ...
            <div v-if="!showResults" class="p-4 sm:p-6">
                ...
                <div class="mb-6">
                    <div class="flex justify-between items-center mb-2">
                        <span class="text-sm text-gray-600">Question {{ currentQuestion + 1 }} of {{
                            test.questions.length }}</span>
                        <span class="text-sm font-medium text-blue-600">{{ Math.round((currentQuestion /
                            test.questions.length) * 100) }}%</span>
                    </div>
                    <div class="bg-gray-200 rounded-full h-2 overflow-hidden">
                        <div class="bg-gradient-to-r from-blue-500 to-purple-500 h-full transition-all duration-300"
                            :style="{ width: `${(currentQuestion / test.questions.length) * 100}%` }"></div>
                    </div>
                </div>
                ...
                <!-- Options -->
                <div class="space-y-3">
                    <button v-for="(option, index) in test.questions[currentQuestion].options" :key="index"
                        @click="selectAnswer(option)" :disabled="answered" :class="[
                        ...
                        <div class="flex items-center gap-3">
                            <span
                                class="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-gray-200 flex items-center justify-center font-bold text-sm flex-shrink-0">
                                {{ String.fromCharCode(65 + index) }}
                            </span>
                            <span class="font-medium text-sm sm:text-base">{{ option }}</span>
                            <span v-if="answered && option === test.questions[currentQuestion].correctAnswer"
                                class="ml-auto text-xl">
                                ✓
                            </span>
                            <span
                                v-if="answered && option === selectedAnswer && option !== test.questions[currentQuestion].correctAnswer"
                                class="ml-auto text-xl">
                                ✗
                            </span>
                        </div>
                ...
                <!-- Feedback -->
                <div v-if="answered" class="mt-6 p-4 rounded-lg"
                ...
                <!-- Next Button -->
                <button v-if="answered" @click="nextQuestion" class="btn-primary w-full mt-6">
                    {{ currentQuestion < test.questions.length - 1 ? 'Next Question →' : 'See Results' }} </button>
            </div>
            ...
            <div v-else class="p-6 sm:p-8">
                <div class="text-center">
                    ...
                    <div class="space-y-3">
                        ...
                        <button v-if="passed" @click="complete" class="btn-primary w-full max-w-sm mx-auto block">
                            Continue Learning
                        </button>
                        <button v-else @click="retryTest" class="btn-primary w-full max-w-sm mx-auto block">
                            Try Again
                        </button>
                        <button @click="$emit('close')" class="btn-secondary w-full max-w-sm mx-auto block">
                            Close
                        </button>
                    </div>
                </div>
